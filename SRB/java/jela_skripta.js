    ...
    function postaviJela() {
        for (let i = 0; i < jela.length; i++) {
            if (jela[i].type == type) {
                ...
                $(telo).append('<p class="cardtext"> Vreme pripreme: '+ jela[i].duration + 'min</p>');
                let tezina;
                switch (jela[i].diff) {
                    case '1': tezina = "Lako"; break;
                    case '2': tezina = "Srednje"; break;
                    case '3': tezina = "Tesko"; break;
                }
                $(telo).append('<p class="cardtext"> Tezina: '+ tezina + '</p>');
                $(telo).append('<p class="cardtext"> Postavio: '+ jela[i].addedBy + '</p>');
                $(telo).append("<br><h5>Komentari:</h5>");
                ...
                $(telo).append(table);
                $(telo).append('<div><br>Dodaj novi komentar<br><textarea cols="40" rows="3" id="kom_' + jela[i].id + '"></textarea><br><button class="btn-success dodaj_kom" id="' + jela[i].id + '">Dodaj</button></div>');
                $(telo).append('<div><br>Ocena: '+ (parseInt(jela[i].sum_graded) * 1.0 / parseInt(jela[i].num_graded)).toFixed(2) +'</div>');
                $(telo).append('<div>Dodaj ocenu: <input type="range" min="1" max="10" id="grade_' + jela[i].id + '"> <button class="btn-success dodaj_grade" id="g_' + jela[i].id  + '">Dodaj</button></div>');
                let download = $("<br><button class='btn-success'>Skini PDF</button>");
